...
const int buttonPins[] = {2, 3, 4, 5}; // Pinos dos botões (A, B, C, D) 2 = A / 5 = D
...
void loop() {
    ...
    bool aPressed = digitalRead(buttonPins[0]) == LOW;
    bool dPressed = digitalRead(buttonPins[3]) == LOW;

    //RESET AO PRESSIONAR "A" + "D"
    if (aPressed && dPressed) {
        delay(debounceDelay); // Pequena pausa para evitar falsos positivos
        if (digitalRead(buttonPins[0]) == LOW && digitalRead(buttonPins[3]) == LOW) {
            Serial.println("reset"); // Envia "reset"
            while (digitalRead(buttonPins[0]) == LOW || digitalRead(buttonPins[3]) == LOW);
            return;  // Sai do loop para evitar que os botões individuais sejam processados
        }
    }
